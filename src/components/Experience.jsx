...
export default function Experience() {
  ...
  return (
    <section id="experience" className="w-full max-w-7xl mx-auto px-4 sm:px-8 py-10 scroll-mt-24">
      <div className="mb-8">
        ...
        <p className="text-sm text-white/50 mt-1">Perjalanan karier dan pendidikan yang membentuk cara saya bekerja.</p>
        ...
        <div className="flex items-center gap-1 mt-4 glass-pill p-1 w-max">
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium transition-all ${
              tab === 'kerja'
                ? 'bg-glass-highlight text-white shadow-sm'
      ...
      <motion.div
        ...
        className="relative space-y-4 before:absolute before:top-2 before:bottom-2 before:left-[19px] sm:before:left-[23px] before:w-px before:bg-gradient-to-b before:from-[#7c6cf0] before:via-[#4f9fd8] before:to-[#7ad7c8]"
      >
        {items.map((item) => {
          ...
          return (
            ...
            >
              {/* Timeline dot */}
              <span className="absolute left-3 sm:left-4 top-5 w-3.5 h-3.5 rounded-full bg-gradient-to-br from-[#7c6cf0] to-[#4f9fd8] border border-white/50 shadow-[0_0_10px_rgba(124,108,240,0.6)]" />

              <div className="glass-card rounded-2xl p-5 sm:p-6 spotlight-card">
                <div className="flex flex-wrap items-center justify-between gap-2 mb-1.5">
                  <h4 className="text-base font-semibold text-white/95">{title}</h4>
                  <span className="flex items-center gap-1.5 text-xs text-white/45 font-mono flex-shrink-0">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {item.tech.map((t) => (
                    <span
                      key={t}
                      className="px-2.5 py-1 rounded-md bg-white/8 border border-white/10 text-[13px] text-white/60"
                    >
                      {t}
                    </span>
                  ))}
