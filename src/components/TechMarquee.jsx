...
export default function TechMarquee() {
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-8 py-8 overflow-hidden" aria-hidden>
      <div className="relative">
        <div className="marquee-track flex items-center gap-4 w-max">
          {row.map((item, i) => (
            <span key={`${item}-${i}`} className="flex items-center gap-4">
              <span className="glass-pill px-5 py-2.5 text-sm font-medium text-white/80 whitespace-nowrap">{item}</span>
              <Code2 className="w-4 h-4 text-white/40 flex-shrink-0" />
            </span>
          ))}
        </div>
